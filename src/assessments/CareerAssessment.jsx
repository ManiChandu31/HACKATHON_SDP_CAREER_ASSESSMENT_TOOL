...
function CareerAssessment() {
  ...
  useEffect(() => {
    ...
    const publishedQuestions = JSON.parse(localStorage.getItem("careerQuestions")) || [];
    ...
    setTimeRemaining(schedule.duration * 60); // Convert to seconds
    setLoading(false);
                ...
                {item.options && item.options.map((option, optIndex) => (
                  <button
                    ...
                    className={`assessment-option ${
                      answers[index] === String(optIndex + 1) ? "selected" : ""
                    }`}
                    onClick={() => handleAnswerChange(index, String(optIndex + 1))}
                  >
                    {option.text}
                  </button>
                ))}
